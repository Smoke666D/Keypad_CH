...
#define CO_LOCK_CAN_SEND(CAN_MODULE)          vPortEnterCritical() //  do { (CAN_MODULE)->primask_send = __get_PRIMASK(); __disable_irq(); } while (0)
#define CO_UNLOCK_CAN_SEND(CAN_MODULE)        vPortExitCritical()// __set_PRIMASK((CAN_MODULE)->primask_send)

/* (un)lock critical section in CO_errorReport() or CO_errorReset() */
#define CO_LOCK_EMCY(CAN_MODULE)               vPortEnterCritical()// do { (CAN_MODULE)->primask_emcy = __get_PRIMASK(); __disable_irq(); } while (0)
#define CO_UNLOCK_EMCY(CAN_MODULE)              vPortExitCritical()// __set_PRIMASK((CAN_MODULE)->primask_emcy)

/* (un)lock critical section when accessing Object Dictionary */
#define CO_LOCK_OD(CAN_MODULE)                 vPortEnterCritical()// do { (CAN_MODULE)->primask_od = __get_PRIMASK(); __disable_irq(); } while (0)
#define CO_UNLOCK_OD(CAN_MODULE)               vPortExitCritical()//  __set_PRIMASK((CAN_MODULE)->primask_od)
